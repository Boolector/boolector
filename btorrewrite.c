...
is_const_one_exp (Btor *btor, BtorNode *exp)
{
  ...
  if (!BTOR_IS_BV_CONST_NODE (BTOR_REAL_ADDR_NODE (exp))) return false;
                        ...
                        BtorNode *e1)
{
  ...
  switch (kind)
  {
    ...
    case BTOR_BEQ_NODE: bresult = btor_eq_bv (mm, b0, b1); break;
...
rewrite_eq_exp (Btor *btor, BtorNode *e0, BtorNode *e1)
{
  ...
  if (btor_get_opt (btor, BTOR_OPT_RW_NORMALIZE)) normalize_eq (btor, &e0, &e1);
...
rewrite_ult_exp (Btor *btor, BtorNode *e0, BtorNode *e1)
{
  ...
  e0 = btor_copy_exp (btor, e0);
  e1 = btor_copy_exp (btor, e1);
  if (btor_get_opt (btor, BTOR_OPT_RW_NORMALIZE))
