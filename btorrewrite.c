...
static BtorNode *binarinorm (Btor *,
                             BtorNode *,
                             BtorNode *,
...
btor_rewrite_add_exp (Btor *btor, BtorNode *e0, BtorNode *e1)
{
      ...
      && (temp = BTOR_REAL_ADDR_NODE (e0))->kind == BTOR_MUL_NODE)
  {
    ...
    else if (BTOR_IS_BV_CONST_NODE (BTOR_REAL_ADDR_NODE (e01)))
    {
      // ~(e00 * c) + e1 == (e00 * (-c) - 1)  + e1
      //
      ...
      && (temp = BTOR_REAL_ADDR_NODE (e1))->kind == BTOR_MUL_NODE)
  {
    ...
    else if (BTOR_IS_BV_CONST_NODE (BTOR_REAL_ADDR_NODE (e11)))
    {
      // e0 + ~(e10 * c) == e0 + (e10 * (-c) - 1)
      //
...
condrewrite (Btor *btor, BtorNode *node, BtorNode *eq)
{
  ...
  if (!lhsvar && !rhsvar) return btor_copy_exp (btor, node);
      ...
      else if (fptr != btor_rewrite_udiv_exp && fptr != btor_rewrite_urem_exp)
      {
        ...
        else
        {
          ...
          if (e_if->kind == BTOR_ADD_NODE)
            normalize_adds_exp (btor, e_if, e_else, &e_if_norm, &e_else_norm);
          else
            normalize_binary_comm_ass_exp (
