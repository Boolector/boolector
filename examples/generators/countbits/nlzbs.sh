...
# hacker's delight, page 78, binary search 
...
((idn = idone))
sr=0
for ((i = $numbits / 2; i > 1; i /= 2))
do
  ((sr += i))
  echo "$((id++)) constd $numbitslog $sr"
  ((lastid = id - 1))
...
((idconst = id - 1))
echo "$((id++)) eq 1 $idxorig $idzero"
((lastid = id - 1))
echo "$((id++)) cond $numbits $lastid $idconst $idn"
((idresult1 = id - 1))
