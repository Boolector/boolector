...
main (int argc, char **argv)
{
  ...
  BtorExp *result, *one, *b_xor_d;

  if (argc != 2)
  {
    printf ("Usage: ./maxxor <num-bits>\n");
    return 1;
  }
  ...
  b_xor_d = boolector_xor (btor, b, d);

  result = btor_maxxor (btor, a, b, c, d, m, num_bits);

  /* conclusion: result is indeed the maximum of b ^ d */
  formula = boolector_ugte (btor, result, b_xor_d);
  ...
  boolector_release (btor, formula);
  boolector_release (btor, b_xor_d);
