...
void
btor_delete_aig_map (Btor *btor, BtorAIGMap *map)
{
  assert (btor);
  assert (map);
...
BtorAIG *
btor_mapped_aigvec (BtorAIGVecMap *map, BtorAIGVec *av)
{
  assert (map);
  assert (av);
  ...
  bucket = btor_find_in_ptr_hash_table (map, av);
  if (!bucket) return 0;
  assert (bucket->key == av);
  return bucket->data.asPtr;
}

void
btor_map_aigvec (Btor *btor,
                 BtorAIGVecMap *map,
                 BtorAIGVec *src,
                 BtorAIGVec *dst)
{
  ...
  assert (bucket->key == src);
  assert (!bucket->data.asPtr);
  bucket->data.asPtr = dst;
}

void
btor_delete_aigvec_map (Btor *btor, BtorAIGMap *map)
{
  assert (btor);
  assert (map);
