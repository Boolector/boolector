	      ...
	      BtorBitVector * bv)
{
   ...
   * -> remove */
    ...
    {
      btor_remove_int_hash_map (roots, -exp->id, 0);
      assert (btor_is_false_bv (btor_get_bv_model (btor, BTOR_INVERT_NODE (exp))));
...
update_cone (Btor * btor, BtorNode * exp, BtorBitVector * assignment)
{
  ...
  while (btor_has_next_node_hash_table_iterator (&it))
	...
	assert (btor_contains_int_hash_map (
		    slv->roots, BTOR_GET_ID_NODE (root)));
      else
	assert (!btor_contains_int_hash_map (
		     slv->roots, BTOR_GET_ID_NODE (root)));
    }
  ...
  while (!BTOR_EMPTY_STACK (stack))
	...
	BTOR_PUSH_STACK (
	    mm, stack, btor_next_parent_iterator (&nit));
    }
  ...
  /* update cone */
  qsort (cone.start, BTOR_COUNT_STACK (cone), sizeof (BtorNode *),
  ...
  for (i = 0; i < BTOR_COUNT_STACK (cone); i++)
	...
	{
	  case BTOR_ADD_NODE:
	    bv = btor_add_bv (mm, e[0], e[1]);
	    break;
	  case BTOR_AND_NODE:
	    bv = btor_and_bv (mm, e[0], e[1]);
	    break;
	  case BTOR_BV_EQ_NODE:
	    bv = btor_eq_bv (mm, e[0], e[1]);
	    break;
	  case BTOR_ULT_NODE:
	    bv = btor_ult_bv (mm, e[0], e[1]);
	    break;
	  case BTOR_SLL_NODE:
	    bv = btor_sll_bv (mm, e[0], e[1]);
	    break;
	  case BTOR_SRL_NODE:
	    bv = btor_srl_bv (mm, e[0], e[1]);
	    break;
	  case BTOR_MUL_NODE:
	    bv = btor_mul_bv (mm, e[0], e[1]);
	    break;
	  case BTOR_UDIV_NODE:
	    bv = btor_udiv_bv (mm, e[0], e[1]);
	    break;
	  case BTOR_UREM_NODE:
	    bv = btor_urem_bv (mm, e[0], e[1]);
	    break;
	  case BTOR_CONCAT_NODE:
	    bv = btor_concat_bv (mm, e[0], e[1]);
	    break;
	  ...
	  default:
	    ...
	    bv = btor_is_true_bv (e[0])
	      ? btor_copy_bv (mm, e[1]) : btor_copy_bv (mm, e[2]);
	}
      ...
      /* update roots table */
      if (cur->constraint
	  || btor_get_ptr_hash_table (btor->assumptions, cur)
...
move (Btor *btor, uint32_t nmoves)
{
  ...
  do
  {
    slv->stats.props += btor_select_move_prop (btor, root, &input, &assignment);
  } while (!input);
