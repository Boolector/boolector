...
// Less function object for BitVector::Bit type. Useful for declaring
// ordered STL containers on this type.
namespace std {
template <>
struct less<BitVector::Bit>
    : public binary_function<BitVector::Bit, BitVector::Bit, bool>
{
  bool operator() (const BitVector::Bit& firstBit,
                   const BitVector::Bit& secondBit) const
  {
    if (firstBit.m_nId < secondBit.m_nId)
    {
      return true;
    }

    if (secondBit.m_nId < firstBit.m_nId)
    {
      return false;
    }

    if (firstBit.m_nBit < secondBit.m_nBit)
    {
      return true;
    }

    if (secondBit.m_nBit < firstBit.m_nBit)
    {
      return false;
    }
...
// Less function object for BitVector::BitRange type. Useful for declaring
// ordered STL containers on this type.
namespace std {
template <>
struct less<BitVector::BitRange>
    : public binary_function<BitVector::BitRange, BitVector::BitRange, bool>
{
  bool operator() (const BitVector::BitRange& firstBitRange,
                   const BitVector::BitRange& secondBitRange) const
  {
    if (firstBitRange.m_nId < secondBitRange.m_nId)
    {
      return true;
    }

    if (secondBitRange.m_nId < firstBitRange.m_nId)
    {
      return false;
    }

    if (firstBitRange.m_nMsb < secondBitRange.m_nMsb)
    {
      return true;
    }

    if (secondBitRange.m_nMsb < firstBitRange.m_nMsb)
    {
      return false;
    }

    if (firstBitRange.m_nLsb < secondBitRange.m_nLsb)
    {
      return true;
    }

    if (secondBitRange.m_nLsb < firstBitRange.m_nLsb)
    {
      return false;
    }
...
// Hash function object for BitVector::Bit types. Useful for declaring STL hash
// associative containers (hash_map, hash_multimap, hash_set and hash_multiset)
// on this type.
class BitVectorBitHashCompare
{
 public:
  // This enum definition is needed for VC++ STL only
  enum
  {
    bucket_size = 4,
    min_buckets = 8
  ...
  size_t operator() (const BitVector::Bit& bit) const
  {
    return (bit.m_nId ^ (bit.m_nBit << 5));
  }

  // This operator is needed for VC++ STL only
  bool operator() (const BitVector::Bit& firstBit,
                   const BitVector::Bit& secondBit) const
  {
    return std::less<BitVector::Bit> () (firstBit, secondBit);
  }
};

// Hash function object for BitVector::BitVector types. Useful for declaring
// STL hash associative containers (hash_map, hash_multimap, hash_set and
// hash_multiset) on this type.
class BitVectorBitRangeHashCompare
{
 public:
  // This enum definition is needed for VC++ STL only
  enum
  ...
  size_t operator() (const BitVector::BitRange& bitRange) const
  {
    return (bitRange.m_nId ^ (bitRange.m_nMsb << 10) ^ (bitRange.m_nLsb << 5));
  }

  // This operator is needed for VC++ STL only
  bool operator() (const BitVector::BitRange& firstRange,
                   const BitVector::BitRange& secondRange) const
  {
    return std::less<BitVector::BitRange> () (firstRange, secondRange);
  }
};

// This operator has been implemented for debug purposes
ostream& operator<< (ostream&, const BitVector::BitRange&);
