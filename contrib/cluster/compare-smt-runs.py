...
FILTER_LOG = [
  ...
  ('sls_num_moves', 
   'MOVES',
   lambda x: 'sls moves' in x,
   lambda x: select_column(x, 3)),
  ('sls_num_flips', 
   'FLIPS',
   lambda x: 'sls flips' in x,
   lambda x: select_column(x, 3)),
  ('sls_num_props',
   'PROPS',
   lambda x: 'sls propagation moves' in x,
   lambda x: select_column(x, 4)),
  ('sls_num_conf_rec', 
   'CONF(REC)',
   lambda x: 'sls propagation move conflicts (recoverable)' in x,
   lambda x: select_column(x, 6)),
  ('sls_num_conf_non_rec',
   'CONF(NON-REC)',
   lambda x: 'sls propagation move conflicts (non-recoverable)' in x,
   lambda x: select_column(x, 6)),
  # prop stats
  ('prop_num_moves', 
   'MOVES',
