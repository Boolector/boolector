...
static BtorNode *apply_beta_reduction (BtorNode *, BtorNode *);
                  ...
                  BtorNode *acc2)
{
  ...
  assert (!BTOR_IS_ARRAY_NODE (acc1));
  ...
  if (BTOR_IS_LAMBDA_NODE (acc2))
  {
    ...
    b = apply_beta_reduction (acc2, i);
    /* b must not be parameterized, otherwise the conflict would not have
     * occured at acc2 */
    assert (!BTOR_IS_PARAMETERIZED_NODE (b));

    // TODO: beta reduction and bfs in one pass?
    cur = NULL;
    bfs_lambda (btor, acc2, acc1, b, &cur);
    assert (cur == NULL);

    prev = NULL;
    /* additionally collect bv conditions from b to acc2 */
    for (cur = b; cur && cur != acc2; cur = cur->parent)
    {
...
synthesize_exp (Btor *btor, BtorNode *exp, BtorPtrHashTable *backannotation)
{
  ...
  fprintf (stderr, "[debug] synthesize exp: ");
...
lazy_synthesize_and_encode_acc_exp (Btor *btor, BtorNode *acc, int force_update)
{
  ...
  fprintf (stderr, "[debug] lazy synthesize exp: ");
...
eval_exp (Btor *btor, BtorNode *exp)
{
  ...
  switch (exp->kind)
  {
    case BTOR_BV_CONST_NODE: { return BTOR_BITS_NODE (btor->mm, exp);
    }
    case BTOR_BV_VAR_NODE:
    {
      assert (exp->tseitin);
    ...
    case BTOR_PARAM_NODE: { return eval_exp (exp->assigned_exp);
    }
    case BTOR_READ_NODE:
    {
      assert (exp->tseitin);
      assert (exp->av);
      return btor_assignment_aigvec (btor->avmgr, exp->av);
    }
    default:
    {
      if (exp->tseitin)
      ...
      switch (exp->kind)
      {
        case BTOR_SLICE_NODE:
        {
          return btor_slice_const (btor->mm,
                                   eval_exp (BTOR_REAL_ADDR_NODE (exp->e[0])),
                                   exp->upper,
                                   exp->lower);
        }
        case BTOR_AND_NODE:
        {
          return btor_and_const (btor->mm,
                                 eval_exp (BTOR_REAL_ADDR_NODE (exp->e[0])),
                                 eval_exp (BTOR_REAL_ADDR_NODE (exp->e[1])));
        }
        case BTOR_BEQ_NODE:
        {
          return btor_cmp_const (btor->mm,
                                 eval_exp (BTOR_REAL_ADDR_NODE (exp->e[0])),
                                 eval_exp (BTOR_REAL_ADDR_NODE (exp->e[1])))
                         == 0
                     ? "1"
        ...
        case BTOR_ADD_NODE:
        {
          return btor_add_const (btor->mm,
                                 eval_exp (BTOR_REAL_ADDR_NODE (exp->e[0])),
                                 eval_exp (BTOR_REAL_ADDR_NODE (exp->e[1])));
        }
        case BTOR_MUL_NODE:
        {
          return btor_mul_concat (btor->mm,
                                  eval_exp (BTOR_REAL_ADDR_NODE (exp->e[0])),
                                  eval_exp (BTOR_REAL_ADDR_NODE (exp->e[1])));
        }
        case BTOR_ULT_NODE:
        ...
        case BTOR_SLL_NODE:
        {
          return btor_sll_const (btor->mm,
                                 eval_exp (BTOR_REAL_ADDR_NODE (exp->e[0])),
                                 eval_exp (BTOR_REAL_ADDR_NODE (exp->e[1])));
        }
        case BTOR_SRL_NODE:
        {
          return btor_srl_const (btor->mm,
                                 eval_exp (BTOR_REAL_ADDR_NODE (exp->e[0])),
                                 eval_exp (BTOR_REAL_ADDR_NODE (exp->e[1])));
        }
        case BTOR_UDIV_NODE:
        {
          return btor_udiv_const (btor->mm,
                                  eval_exp (BTOR_REAL_ADDR_NODE (exp->e[0])),
                                  eval_exp (BTOR_REAL_ADDR_NODE (exp->e[1])));
        }
        case BTOR_UREM_NODE:
        {
          return btor_urem_const (btor->mm,
                                  eval_exp (BTOR_REAL_ADDR_NODE (exp->e[0]));
                                  eval_exp (BTOR_REAL_ADDR_NODE (exp->e[1])));
        }
        ...
        case BTOR_BCOND_NODE:
        {
          return eval_exp (BTOR_REAL_ADDR_NODE (exp->e[0])) == "1"
                     ? eval_exp (BTOR_REAL_ADDR_NODE (exp->e[1]))
                     : eval_exp (BTOR_REAL_ADDR_NODE (exp->e[2]));
        }
        default:
        {
    ...
    *assignments_changed = lazy_synthesize_and_encode_acc_exp (btor, acc, 1);
    ...
    else
    {
      ...
      if (bucket)
      {
        ...
        if (compare_assignments (hashed_value, value) != 0)
        {
          ...
          fprintf (stderr, "[debug]   array: ");
          dump_node (stderr, array);
          fprintf (stderr, "[debug]   acc1: ");
          dump_node (stderr, hashed_acc);
          fprintf (stderr, "[debug]   acc2: ");
          dump_node (stderr, acc);
          btor->stats.array_axiom_1_conflicts++;
    ...
    if (BTOR_IS_WRITE_NODE (array))
    {
      *assignments_changed =
      ...
      {
        fprintf (stderr,
                 "[debug] array axiom 2 conflict in array expression: ");
        dump_node (stderr, array);
        fprintf (stderr, "[debug] add_lemma:\n");
        fprintf (stderr, "[debug]   array: ");
        dump_node (stderr, array);
    ...
    else if (BTOR_IS_LAMBDA_NODE (array))
    {
      *assignments_changed =
      ...
      else
      {
        ...
        {
          fprintf (stderr,
                   "[debug] array axiom 2 conflict in lambda expression: ");
          dump_node (stderr, array);
          fprintf (stderr, "[debug] add_lemma:\n");
          fprintf (stderr, "[debug]   array: ");
          dump_node (stderr, array);
