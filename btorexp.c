                  ...
                  BtorNode *acc2)
{
  ...
  if (BTOR_IS_LAMBDA_NODE (acc2))
  {
    ...
    assign_param (acc2, i);
    lambda_value = beta_reduce (btor, acc2, 0, -1, &parameterized);
    unassign_param (acc2);
    b            = lambda_value;
    ...
    Btor *btor, int elem_len, int index_len, BtorNode *e_param, BtorNode *e_exp)
{
  ...
  if (!btor_find_in_ptr_hash_table (btor->lambda_exps, lambda_exp))
  ...
  do
  {
    ...
    if (propagate_writes_as_reads)
    {
      ...
      while (!BTOR_EMPTY_STACK (param_read_stack))
      {
        ...
        assign_param (lambda_exp, index);
        lambda_value = beta_reduce (btor, lambda_exp, 0, -1, &parameterized);
        unassign_param (lambda_exp);
             ...
             int *parameterized)
{
  ...
  while (!BTOR_EMPTY_STACK (work_stack))
  {
    ...
    if (real_cur->beta_mark == 0 || real_cur->beta_mark == 2)
    {
  ...
  while (!BTOR_EMPTY_STACK (*stack))
  {
    ...
    else if (BTOR_IS_LAMBDA_NODE (array))
    {
      *assignments_changed =
      ...
      lambda_value = beta_reduce (btor, array, 0, -1, &parameterized);
