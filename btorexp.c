                               ...
                               BtorExpKind kind)
{
  ...
  b      = comm->first;
  common = copy_exp (btor, (BtorExp *) b->key);
  if (b->data.asInt > 0)
    b->data.asInt--;
  else
    b = b->next;
...
new_slice_exp_node (Btor *btor, BtorExp *e0, int upper, int lower)
{
  ...
  if (btor->rewrite_level < 2 || b0 == NULL) return (BtorExp *) exp;
  ...
  exp->bits = btor_slice_const_3vl (mm, b0, upper, lower);
    ...
    Btor *btor, BtorExpKind kind, BtorExp *e0, BtorExp *e1, int len)
{
  ...
  if (btor->rewrite_level < 2 || b0 == NULL || b1 == NULL
  ...
  switch (kind)
  {
    case BTOR_AND_EXP: exp->bits = btor_and_const_3vl (mm, b0, b1); break;
    case BTOR_BEQ_EXP: exp->bits = btor_eq_const_3vl (mm, b0, b1); break;
    case BTOR_ADD_EXP: exp->bits = btor_add_const_3vl (mm, b0, b1); break;
    case BTOR_MUL_EXP: exp->bits = btor_mul_const_3vl (mm, b0, b1); break;
    case BTOR_ULT_EXP: exp->bits = btor_ult_const_3vl (mm, b0, b1); break;
    case BTOR_SLL_EXP: exp->bits = btor_sll_const_3vl (mm, b0, b1); break;
    case BTOR_SRL_EXP: exp->bits = btor_srl_const_3vl (mm, b0, b1); break;
    case BTOR_UDIV_EXP: exp->bits = btor_udiv_const_3vl (mm, b0, b1); break;
    case BTOR_UREM_EXP: exp->bits = btor_urem_const_3vl (mm, b0, b1); break;
    default:
      assert (kind == BTOR_CONCAT_EXP);
      exp->bits = btor_concat_const_3vl (mm, b0, b1);
      break;
  }
                      ...
                      int len)
{
  ...
  if (btor->rewrite_level < 2 || b0 == NULL || b1 == NULL || b2 == NULL)
    return (BtorExp *) exp;
  ...
  exp->bits = btor_cond_const_3vl (mm, b0, b1, b2);
...
rewrite_slice_exp_bounded (
    Btor *btor, BtorExp *e0, int upper, int lower, int *calls)
{
  BtorMemMgr *mm;
...
eq_exp_bounded (Btor *btor, BtorExp *e0, BtorExp *e1, int *calls)
{
  ...
  if (btor->rewrite_level > 0)
  {
    ...
    if (is_always_unequal_rwl1 (btor, e0, e1)) return false_exp (btor);
    ...
    if (btor->rewrite_level > 2)
    {
      if (!BTOR_IS_INVERTED_EXP (e0))
      {
        ...
        if (e0->kind == BTOR_ADD_EXP)
        {
          ...
          {
            *calls += 1;
            left   = zero_exp (btor, e0->len);
            result = eq_exp_bounded (btor, left, e0->e[1], calls);
          ...
          if (e0->e[1] == e1)
          {
            *calls += 1;
            left   = zero_exp (btor, e0->len);
            result = eq_exp_bounded (btor, left, e0->e[0], calls);
            release_exp (btor, left);
            return result;
          }
        }
        ...
        {
          if (is_always_unequal_rwl1 (btor, e0->e[1], e1))
          {
            *calls += 1;
          ...
          if (is_always_unequal_rwl1 (btor, e0->e[2], e1))
          {
            *calls += 1;
      ...
      if (!BTOR_IS_INVERTED_EXP (e1))
      {
        ...
        if (e1->kind == BTOR_ADD_EXP)
        {
          ...
          {
            *calls += 1;
            left   = zero_exp (btor, e1->len);
            result = eq_exp_bounded (btor, left, e1->e[1], calls);
            release_exp (btor, left);
            return result;
          }
          ...
          if (e1->e[1] == e0)
          {
            *calls += 1;
            left   = zero_exp (btor, e1->len);
            result = eq_exp_bounded (btor, left, e1->e[0], calls);
            release_exp (btor, left);
            return result;
          }
        }
        ...
        {
          if (is_always_unequal_rwl1 (btor, e1->e[1], e0))
          {
            *calls += 1;
          ...
          if (is_always_unequal_rwl1 (btor, e1->e[2], e0))
          {
            *calls += 1;
...
{
  int calls = 0;
  return cond_exp_bounded (btor, e_cond, e_if, e_else, &calls);
}
...
{
  int calls = 0;
  return mul_exp_bounded (btor, e0, e1, &calls);
}
                             ...
                             int *bound_ptr)
{
  ...
  *bound_ptr -= 1;
  ...
  else if (term->kind == BTOR_ADD_EXP)
  {
    ...
    *rhs_ptr = add_exp (btor, other, tmp);
