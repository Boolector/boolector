...
btor_reduce (Btor *btor, BtorNode *exp)
{
  ...
  return beta_reduce (btor, exp, 0, &parameterized);
}
...
btor_apply_and_reduce (Btor *btor, int argc, BtorNode **args, BtorNode *lambda)
{
  ...
  result = beta_reduce (btor, lambda, 0, &parameterized);
...
beta_reduce (Btor *btor, BtorNode *exp, int bound, BtorNode **parameterized)
{
  ...
  BtorNodePtrStack parameterized_stack;
    ...
    else if (BTOR_IS_LAMBDA_NODE (array))
    {
      *assignments_changed =
      ...
      if (parameterized)
      {
        ...
        values_equal = strcmp (value_assignment, lambda_value_assignment) == 0;
