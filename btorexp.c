...
static BtorNode *beta_reduce (Btor *, BtorNode *, int, BtorNode **);
...
remove_from_hash_tables (Btor *btor, BtorNode *exp)
{
  ...
  switch (exp->kind)
  {
    ...
    case BTOR_ACOND_NODE:
      btor_remove_from_ptr_hash_table (btor->aconds, exp, 0, 0);
      break;
                    ...
                    int overwrite)
{
  ...
  BTORLOG ("%s: %s -> %s (overwrite: %d)",
           __FUNCTION__,
           node2string (exp),
           node2string (simplified),
           overwrite);
...
merge_simplified_exp_const (Btor *btor, BtorNode *a, BtorNode *b)
{
  ...
  if (a != rep)
  {
    /* top level constraint, force to true */
                    ...
                    BtorNode *a_else)
{
  ...
  if (!btor_find_in_ptr_hash_table (btor->aconds, exp))
    (void) btor_insert_in_ptr_hash_table (btor->aconds, exp);
...
btor_apply_and_reduce (Btor *btor, int argc, BtorNode **args, BtorNode *lambda)
{
  ...
  result = beta_reduce (btor, lambda, BETA_RED_FULL, &parameterized);
...
btor_new_btor (void)
{
  ...
  btor->aux_hash_table =
      btor_new_ptr_hash_table (mm,
                               (BtorHashPtr) btor_hash_exp_by_id,
                               (BtorCmpPtr) btor_compare_exp_by_id);
  btor->aconds            = btor_new_ptr_hash_table (mm,
                                          (BtorHashPtr) btor_hash_exp_by_id,
                                          (BtorCmpPtr) btor_compare_exp_by_id);
  btor->bv_lambda_id      = 1;
...
btor_delete_btor (Btor *btor)
{
  ...
  btor_delete_ptr_hash_table (btor->aconds);
...
unassign_param (Btor *btor, BtorNode *lambda)
{
  ...
  do
  {
    BTORLOG ("unassign_param: %s", node2string (lambda));
  ...
  while (!BTOR_EMPTY_STACK (work_stack))
  {
    ...
    if (real_cur->eval_mark == 0 || real_cur->eval_mark == 2)
    {
      ...
      if (BTOR_IS_PARAM_NODE (real_cur))
      {
        ...
        assigned_exp = param_cur_assignment (real_cur);
 ...
 */
static BtorNode *
beta_reduce (Btor *btor, BtorNode *exp, int bound, BtorNode **parameterized)
{
  assert (btor);
  assert (exp);
  ...
  double start;
  BtorMemMgr *mm;
  BtorNode *cur, *real_cur, *next, *e[3], *result, *param, *p[3];
    ...
    if (mark == 0)
    {
                  ...
                  || ((BtorLambdaNode *) e[0])->chain_depth > 0)))
      {
        ...
        if (BTOR_IS_PARAM_NODE (BTOR_REAL_ADDR_NODE (e[1]))
            && param_cur_assignment (e[1]))
        {
          assign_param (btor, e[0], param_cur_assignment (e[1]));
        }
        else
        {
      ...
      else
      {
        ...
        switch (real_cur->kind)
        {
          ...
          case BTOR_LAMBDA_NODE:
            ...
            // TODO: may be a rewriting rule for lambdas while creating
            //       if lambda body is constant return body instead of
            //       creating a constant lambda
      ...
      *assignments_changed =
      ...
      if (parameterized)
      {
        ...
        else
        {
          ...
          lambda_value_assignment = (char *) eval_exp (btor, lambda_value);
	...
	*assignments_changed =
      ...
      else if (BTOR_IS_LAMBDA_NODE (array))
	{
	  *assignments_changed =
	  ...
	  assign_param (btor, array, index);
	  lambda_value = 
	    beta_reduce (btor, array, BETA_RED_CUTOFF, &parameterized);
	  unassign_param (btor, array);
...
rebuild_exp (Btor *btor, BtorNode *exp, int rww, int rwr)
{
  ...
  switch (exp->kind)
  {
    ...
    case BTOR_READ_NODE:
      ...
      if (rwr) return beta_reduce (btor, exp, BETA_RED_FULL, &parameterized);
      return btor_read_exp (btor, exp->e[0], exp->e[1]);
    case BTOR_WRITE_NODE:
      //	if (rww && btor->rewrite_writes)
      if (rww) return rewrite_write_to_lambda_exp (btor, exp);
...
substitute_and_rebuild (Btor *btor, BtorPtrHashTable *subst, int rww, int rwr)
{
  ...
  while (!BTOR_EMPTY_STACK (stack))
  {
    ...
    if (cur->aux_mark == 1)
    {
      ...
      if (cur->simplified) BTOR_PUSH_STACK (mm, stack, cur->simplified);
...
rewrite_reads_on_aconds (Btor *btor)
{
  ...
  int i;
  BtorPtrHashTable *reads, *param_reads;
  BtorPtrHashBucket *b;
  BtorNodePtrQueue queue;
  BtorNode *read, *read_simp, *acond, *e[3], *read_then, *read_else;
  BtorPartialParentIterator it;

  return;

  if (btor->aconds->count == 0) return;

  BTOR_INIT_QUEUE (queue);

  //  assert (btor->aux_hash_table->count == 0u);
  //  reads = btor->aux_hash_table;
  reads = btor_new_ptr_hash_table (btor->mm,
                                   (BtorHashPtr) btor_hash_exp_by_id,
                                   (BtorCmpPtr) btor_compare_exp_by_id);

  param_reads = btor_new_ptr_hash_table (btor->mm,
                                         (BtorHashPtr) btor_hash_exp_by_id,
                                         (BtorCmpPtr) btor_compare_exp_by_id);

  for (b = btor->aconds->last; b; b = b->prev)
  {
    acond = (BtorNode *) b->key;
    assert (BTOR_IS_REGULAR_NODE (acond));
    assert (BTOR_IS_ARRAY_COND_NODE (acond));
    BTOR_ENQUEUE (btor->mm, queue, acond);
  }

  /* collect reads */
  while (!BTOR_EMPTY_QUEUE (queue))
  {
    acond = BTOR_DEQUEUE (queue);
    assert (BTOR_IS_REGULAR_NODE (acond));

    for (i = 0; i < acond->arity; i++) e[i] = acond->e[i];

    // TODO: use refs
    // TODO: restrictions when to simplify
    if (acond->refs > 1) continue;
    ...
    while (has_next_parent_read_parent_iterator (&it))
    {
      ...
      if (!BTOR_IS_READ_NODE (read)) continue;

      assert (!btor_find_in_ptr_hash_table (reads, read));
      ...
      //	  {
      //	    assert (!btor_find_in_ptr_hash_table (reads, read_then));
      //	    btor_insert_in_ptr_hash_table (reads, read_then);
      ////	    assert (!btor_find_in_ptr_hash_table (param_reads,
      /// read_then)); /	    btor_insert_in_ptr_hash_table (param_reads,
      /// read_then);
      //	  }
      //
      ...
      //	  {
      //	    assert (!btor_find_in_ptr_hash_table (reads, read_else));
      //	    btor_insert_in_ptr_hash_table (reads, read_else);
      ////	    assert (!btor_find_in_ptr_hash_table (param_reads,
      /// read_else)); /	    btor_insert_in_ptr_hash_table (param_reads,
      /// read_else);
      //	  }
  ...
  substitute_and_rebuild (btor, reads, 0, 0);
