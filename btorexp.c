...
remove_from_hash_tables (Btor *btor, BtorNode *exp)
{
  ...
  /* Note: Do not remove from aux_hash_table! aux_hash_table is for
   *       temporary use only -> expressions should only be removed if
   *       aux_hash_table would otherwise be inconsistent (btor_release_exp).
   *       Else this should be handled locally.  */
}
...
recursively_release_exp (Btor *btor, BtorNode *root)
{
  ...
  do
  {
    ...
    RECURSIVELY_RELEASE_NODE_ENTER_WITHOUT_POP:
      ...
      /* if aux_hash_table is in use and cur is released, we have to
       * remove it, otherwise aux_hash_table is inconsistent */
      if (btor_find_in_ptr_hash_table (btor->aux_hash_table, cur))
        btor_remove_from_ptr_hash_table (btor->aux_hash_table, cur, 0, 0);
                         ...
                         BtorNode *b)
{
  ...
  BTORLOG ("\e[1;32m", 0);
  BTORLOG ("ENCODED LEMMA", 0);
  BTORLOG ("  index: %c", i, BTOR_IS_INVERTED_NODE (i) ? '-' : ' ');
  BTORLOG ("  premisses:", 0);
  ...
  if (j)
  {
    BTORLOG ("    indices:", 0);
    BTORLOG ("      eq:", 0);
    BTORLOG ("       %c", i, BTOR_IS_INVERTED_NODE (i) ? '-' : ' ');
    BTORLOG ("       %c", j, BTOR_IS_INVERTED_NODE (j) ? '-' : ' ');
  }
  ...
  for (bucket = writes->last; bucket; bucket = bucket->prev)
  {
    ...
    BTORLOG ("      eq:", 0);
    BTORLOG ("       %c", i, BTOR_IS_INVERTED_NODE (i) ? '-' : ' ');
    BTORLOG ("       %c", cur, BTOR_IS_INVERTED_NODE (cur) ? '-' : ' ');
  }
  ...
  BTORLOG ("  conclusion:", 0);
  BTORLOG ("    eq:", 0);
  BTORLOG ("      %c", a, BTOR_IS_INVERTED_NODE (a) ? '-' : ' ');
  BTORLOG ("      %c", b, BTOR_IS_INVERTED_NODE (b) ? '-' : ' ');
  BTORLOG ("\e[0;39m", 0);
}
            ...
            int propagate_upwards)
{
  ...
  BTORLOG ("[bfs_lambda] searching for: ", search);
  ...
  if (propagate_upwards)
  {
    ...
    BTORLOG ("[bfs_lambda up] ", lambda_exp);
    BTORLOG ("             -> ", cur);
  }
  else
  {
    cur->parent = lambda_exp;
    BTORLOG ("[bfs_lambda down] ", cur);
    BTORLOG ("               -> ", lambda_exp);
  }
  cur->mark = 1;
  ...
  do
  {
    ...
    if (BTOR_IS_BV_COND_NODE (cur))
    {
      ...
      if (propagate_upwards)
      {
        ...
        BTORLOG ("[bfs_lambda up] ", cur);
        BTORLOG ("             -> ", next);
      }
      else
      {
        next->parent = cur;
        BTORLOG ("[bfs_lambda down] ", next);
        BTORLOG ("               -> ", cur);
      }
  ...
  // debug
  if (found)
    BTORLOG ("[bfs_lambda] found: ", search);
  else
    BTORLOG ("[bfs_lambda] not found: ", search);
  // debug
    ...
    while (1)
    {
          ...
          || (cur == acc && !BTOR_EMPTY_STACK (bconds)))
      {
        ...
        while (!BTOR_EMPTY_STACK (bconds))
        {
          ...
          BTORLOG ("set lambda exp for: ", bcond);
          BTORLOG ("  lambda exp: ", lambda);
...
lazy_synthesize_and_encode_acc_exp (Btor *btor, BtorNode *acc, int force_update)
{
  ...
  BTORLOG ("lazy_synthesize_and_encode_acc_exp: ", acc);
                                       ...
                                       int force_update)
{
  ...
  BTORLOG ("lazy_synthesize_and_encode_lambda_exp: ", lambda_exp);
...
static void
assign_param (Btor *btor, BtorNode *lambda, BtorNode *arg)
{
  assert (lambda);
  assert (arg);
...
static void
unassign_param (Btor *btor, BtorNode *lambda)
{
  assert (lambda);
  assert (BTOR_IS_REGULAR_NODE (lambda));
  assert (BTOR_IS_LAMBDA_NODE (lambda));
...
eval_exp (Btor *btor, BtorNode *exp)
{
  ...
  BTORLOG ("eval_exp: ", BTOR_REAL_ADDR_NODE (exp));
  ...
  while (!BTOR_EMPTY_STACK (work_stack))
  {
    ...
    BTORLOG ("eval_exp: real_cur: ", real_cur);
...
beta_reduce (Btor *btor, BtorNode *exp, int bound, int *parameterized)
{
  ...
  double start;
  BtorMemMgr *mm;
  BtorNode *cur, *real_cur, *next, *e[3], *result, *param, *assigned_exp;
  BtorNodePtrStack work_stack, arg_stack, assign_stack, unassign_stack;
  BtorIntStack mark_stack, parameterized_stack;
  ...
  while (!BTOR_EMPTY_STACK (work_stack))
  {
      ...
      for (i = 0; i < BTOR_COUNT_STACK (work_stack); i++)
	{
	  fprintf (stderr, "    work_stack[%d]: (mark %d) ", i, 
		   mark_stack.start[i]);
	  dump_node (stderr, work_stack.start[i]);
	}
      fprintf (stderr, "\n");
      for (i = 0; i < BTOR_COUNT_STACK (arg_stack); i++)
	{
	  fprintf (stderr, "    arg_stack[%d]: (param %d) ", i,
		   parameterized_stack.start[i]);
	  dump_node (stderr, arg_stack.start[i]);
	}
      fprintf (stderr, "\n");
      for (i = 0; i < BTOR_COUNT_STACK (assign_stack); i++)
	{
	  fprintf (stderr, "    assign_stack[%d]: ", i);
	  dump_node (stderr, assign_stack.start[i]);
	}
      // end debug
#endif
  ...
  BETA_REDUCE_POP_WORK_STACK:
    ...
    //      fprintf (stderr, "beta_reduce: real_cur: ");
    ...
    if (mark == 0)
    {
              ...
              && BTOR_IS_LAMBDA_NODE (BTOR_REAL_ADDR_NODE (real_cur->e[0]))))
      {
        ...
        if (BTOR_IS_PARAM_NODE (e[1]) && ((BtorParamNode *) e[1])->assigned_exp)
          BTOR_PUSH_STACK (
              mm, assign_stack, ((BtorParamNode *) e[1])->assigned_exp);
        else
          BTOR_PUSH_STACK (mm, assign_stack, e[1]);

        //	      assigned_exp = ((BtorParamNode *) param)->assigned_exp;
        //
        //	      if (!assigned_exp)
        //		{
        //		  BTOR_PUSH_STACK (mm, unassign_stack, e[0]);
        //		  if (BTOR_IS_PARAM_NODE (e[1]) &&
        //		      ((BtorParamNode *) e[1])->assigned_exp)
        //		    assign_param (btor, e[0],
        //			((BtorParamNode *) e[1])->assigned_exp);
        //		  else
        //		    assign_param (btor, e[0], e[1]);
        //		}
      }
      else if (BTOR_IS_PARAM_NODE (real_cur))
      {
        assert (!BTOR_EMPTY_STACK (assign_stack));
        /* we allow unassigned params (next == 0) */
        next = BTOR_TOP_STACK (assign_stack);
      ...
      {
        eval_res = eval_exp (btor, real_cur->e[0]);
        next     = eval_res[0] == '1' ? real_cur->e[1] : real_cur->e[2];
        assert (next);
        //	      fprintf (stderr, "beta_reduce: next ");
      ...
      {
        /* we currently beta-reduce a read on a lambda, reducing the
         * read index is done already -> we have to update the parameter
         * assignment accordingly. */
        if (BTOR_IS_LAMBDA_NODE (real_cur)
            && (!BTOR_EMPTY_STACK (assign_stack)))
        {
          (void) BTOR_POP_STACK (assign_stack);
          BTOR_PUSH_STACK (mm, assign_stack, BTOR_TOP_STACK (arg_stack));
        }

        BTOR_PUSH_STACK (mm, work_stack, cur);
        BTOR_PUSH_STACK (mm, mark_stack, mark);
        /* Note: we beta-reduce the index of a read on a lambda first
         * in order to simplify handling of quasi-cyclic structures
        ...
        for (i = 0; i < real_cur->arity; i++)
        {
              ...
              && BTOR_IS_PARAM_NODE (BTOR_REAL_ADDR_NODE (real_cur->e[i])))
          {
            ...
            BTOR_PUSH_STACK (
                mm, assign_stack, ((BtorParamNode *) param)->assigned_exp);
          }
          BTOR_PUSH_STACK (mm, work_stack, real_cur->e[i]);
          BTOR_PUSH_STACK (mm, mark_stack, 0);
        }
          ...
          case BTOR_LAMBDA_NODE:
            if (BTOR_REAL_ADDR_NODE (e[1])->parameterized)
            {
                         ...
                         != BTOR_REAL_ADDR_NODE (real_cur->e[1]))
              {
                ...
                // restriction for now.
                //
                // Note: rww is currently disabled for ext.
                //
  ...
  assert (BTOR_EMPTY_STACK (mark_stack));
  ...
  *parameterized = BTOR_POP_STACK (parameterized_stack);
  ...
  BTOR_RELEASE_STACK (mm, work_stack);
  BTOR_RELEASE_STACK (mm, arg_stack);
  BTOR_RELEASE_STACK (mm, assign_stack);
  BTOR_RELEASE_STACK (mm, mark_stack);
  BTOR_RELEASE_STACK (mm, parameterized_stack);

  // fprintf (stderr, "* beta_reduce result (%d): ", *parameterized);
  ...
  //BTORLOG ("** beta_reduce: ", BTOR_REAL_ADDR_NODE (exp));
  fprintf (stderr, "** beta_reduce: ");
  dump_node (stderr, exp);
...
//      BTORLOG ("beta_reduce: real_cur (%d): ", real_cur, real_cur->beta_mark);
      fprintf (stderr, "beta_reduce: real_cur (%d): ", real_cur->beta_mark);
      dump_node (stderr, real_cur);
      ...
      if (real_cur->beta_mark != 1)
	{
	  ...
	  if (bound >= 0 && 
	       ...
	       && BTOR_IS_LAMBDA_NODE (BTOR_REAL_ADDR_NODE (real_cur->e[0]))))
	    {
	      ...
	      index = real_cur->e[1];
	      param = BTOR_REAL_ADDR_NODE (
			BTOR_REAL_ADDR_NODE (real_cur->e[0])->e[0]);

	      assert (BTOR_IS_PARAM_NODE (param));

	      if (!((BtorParamNode *) param)->assigned_exp)
		{
		  BTOR_PUSH_STACK (mm, unassign_stack, 
		      BTOR_REAL_ADDR_NODE (real_cur->e[0]));

		  if (BTOR_IS_PARAM_NODE (index) 
		      && ((BtorParamNode *) index)->assigned_exp)
		    {
		      assign_param (btor, BTOR_REAL_ADDR_NODE (real_cur->e[0]),
				    ((BtorParamNode *) index)->assigned_exp);
		    }
		  else
		    {
	      ...
	      switch (real_cur->kind)
		{
		  case BTOR_SLICE_NODE:
		    result = btor_slice_exp (
			       btor, e[0], real_cur->upper, real_cur->lower);
		    break;
		  case BTOR_AND_NODE:
		    result = btor_and_exp (btor, e[0], e[1]);
		    break;
		  case BTOR_BEQ_NODE:
		  case BTOR_AEQ_NODE:
		    result = btor_eq_exp (btor, e[0], e[1]);
		    break;
		  case BTOR_ADD_NODE:
		    result = btor_add_exp (btor, e[0], e[1]);
		    break;
		  case BTOR_MUL_NODE:
		    result = btor_mul_exp (btor, e[0], e[1]);
		    break;
		  case BTOR_ULT_NODE:
		    result = btor_ult_exp (btor, e[0], e[1]);
		    break;
		  case BTOR_SLL_NODE:
		    result = btor_sll_exp (btor, e[0], e[1]);
		    break;
		  case BTOR_SRL_NODE:
		    result = btor_srl_exp (btor, e[0], e[1]);
		    break;
		  case BTOR_UDIV_NODE:
			...
			if (!real_cur->beta_aux_mark
			       ...
			       != BTOR_REAL_ADDR_NODE (real_cur->e[1]))
			  {
				...
				if (param->symbol)
				  {
				    BTOR_NEWN (mm, symbol, 
						   strlen (param->symbol) + 2);
				    sprintf (symbol, "%s'", param->symbol);
				    param = 
				      btor_param_exp (btor, param->len, symbol);
				    btor_freestr (mm, symbol);
				  }
				else
				  {
				    param = 
				      btor_param_exp (btor, param->len, "");
				  }
  ...
  while (!BTOR_EMPTY_STACK (*stack))
  {
    ...
    else
    {
      ...
      if (bucket)
      {
        ...
        {
          BTORLOG ("\e[1;31m", 0);
          BTORLOG ("array axiom 1 conflict in array expression: ", array);
          BTORLOG ("add_lemma:", 0);
          BTORLOG ("  array: ", array);
          BTORLOG ("  acc1: ", hashed_acc);
          BTORLOG ("  acc2: ", acc);
          BTORLOG ("\e[0;39m", 0);
          btor->stats.array_axiom_1_conflicts++;
    ...
    if (BTOR_IS_WRITE_NODE (array))
    {
      *assignments_changed =
      ...
      {
        BTORLOG ("\e[1;31m", 0);
        BTORLOG ("array axiom 2 conflict in array expression: ", array);
        BTORLOG ("add_lemma:", 0);
        BTORLOG ("  array: ", array);
        BTORLOG ("  acc1: ", acc);
        BTORLOG ("  acc2: ", array);
      ...
      else if (!indices_equal)
      {
        ...
        BTORLOG ("write exp prop. down:", 0);
        BTORLOG ("  array: ", array->e[0]);
        BTORLOG ("  acc: ", acc);
      }
    }
    else if (BTOR_IS_ARRAY_COND_NODE (array))
    {
      *assignments_changed =
      ...
      /* propagate down */
      BTOR_PUSH_STACK (mm, *stack, acc);
      BTORLOG ("array cond prop. down:", 0);
      BTORLOG ("  acc: ", acc);
      if (assignment == 1)
      {
    ...
    else if (BTOR_IS_LAMBDA_NODE (array))
    {
      *assignments_changed =
      ...
      BTORLOG ("   lambda: %s, ", BTOR_REAL_ADDR_NODE (lambda_value), a);
      btor_free_bv_assignment_exp (btor, a);
      BTORLOG ("   lambda parameterized: %d", 0, parameterized);
      // end debug
          ...
          && parameterized)
      {
        ...
        BTORLOG ("lambda exp prop. down:", 0);
        BTORLOG ("  array: ", lambda_value->e[0]);
        BTORLOG ("  acc: ", acc);
      }
      else
      {
        ...
        {
          BTORLOG ("\e[1;31m", 0);
          BTORLOG ("array axiom 2 conflict in lambda expression: ", array);
          BTORLOG (" add_lemma:", 0);
          BTORLOG ("   array: ", array);
          BTORLOG ("   acc1: ", acc);
          BTORLOG ("   acc2: ", array);
    ...
    if (propagate_writes_as_reads)
    {
      ...
      while (!BTOR_EMPTY_STACK (param_read_stack))
      {
        ...
        if (BTOR_IS_READ_NODE (lambda_value) && parameterized)
        {
          ...
          assert (lambda_value->e[1] == index);
          BTORLOG ("lambda prop. upwards:", 0);
          BTORLOG ("  access: ", acc);
          BTORLOG ("  array: ", lambda_exp);
          BTOR_PUSH_STACK (mm, *stack, acc);
...
BTOR_READ_WRITE_ARRAY_CONFLICT_CHECK:
  ...
  for (temp = top_arrays->start; temp != top; temp++)
  {
    ...
    BTORLOG ("push top_array (%d): ", cur_array, cur_array->array_mark);
  }

  while (!BTOR_EMPTY_STACK (array_stack))
  {
    cur_array = BTOR_POP_STACK (array_stack);
    BTORLOG ("push cur_array (%d): ", cur_array, cur_array->array_mark);
    assert (BTOR_IS_REGULAR_NODE (cur_array));
    assert (BTOR_IS_ARRAY_NODE (cur_array));
    assert (cur_array->reachable);
    assert (!cur_array->simplified);
