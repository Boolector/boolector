                              ...
                              BtorNodePtrStack *prop_stack)
{
  ...
  do
  {
    ...
    if (cur->mark || !cur->apply_below
        || btor_find_in_ptr_hash_table (slv->searched_applies, cur))
      continue;

    cur->mark = 1;
    BTOR_PUSH_STACK (btor->mm, unmark, cur);
    btor_insert_in_ptr_hash_table (slv->searched_applies, cur);
                                        ...
                                        BtorNodePtrStack *prop_stack)
{
  assert (btor);
  assert (BTOR_CORE_SOLVER (btor));
  assert (BTOR_CORE_SOLVER (btor)->searched_applies);
  assert (exp);
  assert (BTOR_IS_REGULAR_NODE (exp));
  assert (prop_stack);
  assert (check_id_table_mark_unset_dbg (btor));
...
BTOR_CONFLICT_CHECK:
  ...
  if (!slv->searched_applies)
  {
    slv->searched_applies =
        btor_new_ptr_hash_table (btor->mm,
                                 (BtorHashPtr) btor_hash_exp_by_id,
                                 (BtorCmpPtr) btor_compare_exp_by_id);
  }
  ...
  btor_delete_ptr_hash_table (slv->searched_applies);
  slv->searched_applies = 0;
...
clone_core_solver (Btor *clone, Btor *btor, BtorNodeMap *exp_map)
{
  ...
  res->searched_applies = btor_clone_ptr_hash_table (
      clone->mm, slv->searched_applies, btor_clone_key_as_node, 0, exp_map, 0);
