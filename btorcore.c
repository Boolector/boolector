...
/* BV skelton is always encoded, with lazy_synthesize enabled,
           ...
           BtorIntHashTable *apply_search_cache)
{
  ...
  bool prop_down, values_equal, conflict;
  BtorBitVector *bv;
  BtorMemMgr *mm;
  BtorCoreSolver *slv;
  BtorLambdaNode *lambda;
...
btor_sat_btor (Btor *btor, int lod_limit, int sat_limit)
{
  ...
  // FIXME (ma): force lazy_synthesize to 0
  btor->options.lazy_synthesize.val = 0;
