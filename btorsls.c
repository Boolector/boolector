      ...
      if (BTOR_IS_AND_NODE (real_cur))
      {
        ...
        if (BTOR_IS_INVERTED_NODE (cur))
        {
          ...
          if (btor->options.loglevel.val >= 2)
          {
            ...
            BTORLOG (2, "      assignment e[0]: %s", a0);
            BTORLOG (2, "      assignment e[1]: %s", a1);
            btor_freestr (btor->mm, a0);
            btor_freestr (btor->mm, a1);
            BTORLOG (2, "      sls score e[0]: %f", s0);
            BTORLOG (2, "      sls score e[1]: %f", s1);
          }
#endif
          res = s0 > s1 ? s0 : s1;
        }
            ...
            int eidx)
{
  ...
  else
  {
        ...
        || (!btor_compare_bv (bve, bvmax) && !isult))
    {
      ...
      {
        tmp = btor_add_bv (btor->mm, bvmax, neg);
             ...
             int eidx)
{
  ...
  BtorNode *e;
  BtorBitVector *res, *tmp, *one, *neg, *bvmax, *n;
  BtorMemMgr *mm;
#ifndef NDEBUG
  BtorBitVector *tmpdbg;
  int iscon = 0;
    ...
    if (btor_compare_bv (bve, bvurem) > 0)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          {
            btor_free_bv (mm, tmp);
            tmp = btor_add_bv (mm, n, negone);
            btor_free_bv (mm, n);
            n = btor_new_random_range_bv (mm, &btor->rng, bve->width, one, tmp);
          }

          mul  = btor_mul_bv (mm, bve, n);
          neg  = btor_neg_bv (mm, mul);
          tmp2 = btor_add_bv (mm, bvmax, neg);
...
select_prop_move (Btor *btor, BtorNode *root)
{
  ...
  for (;;)
  {
    ...
    if (BTOR_IS_BV_VAR_NODE (BTOR_REAL_ADDR_NODE (real_cur->e[eidx])))
    {
      cur = real_cur->e[eidx];
    ADD_CAN_NEIGH:
      b             = btor_insert_in_ptr_hash_table (slv->max_cans,
                                         BTOR_REAL_ADDR_NODE (cur));
      b->data.asPtr = BTOR_IS_INVERTED_NODE (cur)
